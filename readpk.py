...
for file_path in file_paths:
    ...
    # print(avg_train_loss, avg_test_accuracy)
    all_avg_train_loss.append(avg_train_loss)
    all_avg_test_accuracy.append(avg_test_accuracy)
...
def plot_metrics(metrics, title, ylabel):
    ...
    plt.xlabel('Epoch')
    plt.ylabel(ylabel)
    plt.legend([os.path.basename(path).split('.')[0] for path in file_paths], loc='best')
...
def plot_metrics_time(metrics, title, ylabel, time_per_epoch):
    ...
    for idx, metric in enumerate(metrics):
        ...
        time = epochs * time_per_epoch[idx]
    ...
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path)
    save_path = os.path.join(os.getcwd(), 'graph_result', f'{ylabel}_time.png')
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path)
